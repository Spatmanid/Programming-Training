...
# this second operation does nothing because in the original string S[2] = 'c', which doesn't 
# match x[0] = 'e'.
#
...
# Output: "eeebffff"
# Explanation: "a" starts at index 0 in S, so it's replaced by "eee". "cd" starts at index 2 in S, 
#              so it's replaced by "ffff".
#
# Input: S = "abcd", indexes = [0,2], sources = ["ab","ec"], targets = ["eee","ffff"]
# Output: "eeecd"
# Explanation: "ab" starts at index 0 in S, so it's replaced by "eee". "ec" doesn't starts at index 
#              2 in the original S, so we do nothing.
#
# Solution
#
# In this problem, the key idea is to sort the replacement operations according to the indexes and 
# do the replacement from the last index to the first one.
#
# Complexity Analysis
# This solution has time complexity of O(n logn) because of the sorting, where n is the length of 
# indexes array.
...
def main():
	S, indexes, sources, targets = "abcd", [0,2], ["a","cd"], ["eee","ffff"]
	sol = Solution()
	print(sol.findReplaceString(S, indexes, sources, targets))
	S, indexes, sources, targets = "abcd", [0,2], ["ab","ec"], ["eee","ffff"]
	sol = Solution()
	print(sol.findReplaceString(S, indexes, sources, targets))
